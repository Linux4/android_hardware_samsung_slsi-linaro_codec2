 ...
 */
#include "Exynos_C2_Mpeg2DecComponent.h"
#include "Exynos_Mpeg2Dec_Filter.h"
...
#include "ExynosLog.h"
#undef LOG_TAG
#define LOG_TAG "ExynosC2Mpeg2DecComponent"

constexpr char COMPONENT_NAME[] = "c2.exynos.mpeg2.decoder";
...
ExynosC2FilterManager::FilterModuleInfoList listFilterInfo = {
    {   "mpeg2dec",
        &ExynosC2FilterManager::FilterModule::makeFilterModule<ExynosMpeg2DecFilter>,
...
static C2R MaxPictureSizeSetter(bool mayBlock, C2InterfaceHelper::C2P<C2StreamMaxPictureSizeTuning::output> &me,
    ...
    return DecCommonSetter::MaxPictureSizeSetter(mayBlock, me, size, 2048u, 2048u);
}
...
class ExynosC2Mpeg2DecComponent::Mpeg2DecParamIntf : public ExynosC2DecComponent::VdecCommonParamIntf {
public:
    Mpeg2DecParamIntf(const std::shared_ptr<C2ReflectorHelper> &helper)
        : VdecCommonParamIntf(
                helper,
                COMPONENT_NAME,
                ...
                listFilterInfo) {
        addDecodeDefaultParam(32, 32, 2048, 2048, (320 * 240 * 3 / 2), MaxPictureSizeSetter);
        ...
        addParameter(
                ...
                .withDefault(new C2StreamProfileLevelInfo::input(0u,
                        C2Config::PROFILE_MP2V_MAIN, C2Config::LEVEL_MP2V_MAIN))
                .withFields({
        ...
        addCroppingParam(2048, 2048, 2048, 2048, 2);

        /* extension feature : compressed color */
        addCompressedColor();
    }

    virtual ~Mpeg2DecParamIntf() = default;

    /* getter functions */

private:
    friend class ExynosC2Mpeg2DecComponent;
};

ExynosC2Mpeg2DecComponent::ExynosC2Mpeg2DecComponent(c2_node_id_t id, const std::shared_ptr<Mpeg2DecParamIntf> &intfImpl)
    : ExynosC2DecComponent(std::make_shared<ExynosC2ComponentInterface>(COMPONENT_NAME, id, intfImpl)) {

    mObjName    = "ExynosC2Mpeg2DecComponent";
    mbLogOff    = false;
...
bool ExynosC2Mpeg2DecComponent::init(std::weak_ptr<C2Component> wkComponent) {
...
void ExynosC2Mpeg2DecComponent::onUpdateC2Config(
    ...
    auto mpeg2DecIntf = std::static_pointer_cast<Mpeg2DecParamIntf>(mParamIntf);
...
c2_status_t ExynosC2Mpeg2DecComponent::onSetup() {
    ExynosLogFunctionTrace();
    ...
    mpeg2DecIntf->addStreamCrop(2048, 2048);
...
class ExynosC2Mpeg2DecFactory : public C2ComponentFactory {
public:
    ExynosC2Mpeg2DecFactory()
    ...
    c2_status_t createComponent(
        ...
        using __component_type = ExynosC2ComponentWrapper<ExynosC2Mpeg2DecComponent,
                                                          ExynosC2Mpeg2DecComponent::Mpeg2DecParamIntf>;

        *component = std::shared_ptr<C2Component>(
                        new __component_type(id, std::make_shared<ExynosC2Mpeg2DecComponent::Mpeg2DecParamIntf>(mHelper)), deleter);

        auto mpeg2Component = std::static_pointer_cast<__component_type>(*component);

        mpeg2Component->init((*component));
    ...
    c2_status_t createInterface(
        ...
        *interface = std::shared_ptr<C2ComponentInterface>(
                        new ExynosC2ComponentInterface(COMPONENT_NAME, id, std::make_shared<ExynosC2Mpeg2DecComponent::Mpeg2DecParamIntf>(mHelper)), deleter);

        return C2_OK;
    }

    virtual ~ExynosC2Mpeg2DecFactory() override = default;
...
EXYNOS_EXPORT_REF extern "C" ::C2ComponentFactory* CreateCodec2Factory() {
    return new ExynosC2Mpeg2DecFactory();
}
