...
#include "ExynosLog.h"
#undef LOG_TAG
#define LOG_TAG "ExynosC2FilterPlugin"
...
class ExynosToneMappingFilter
    : public C2Component, public std::enable_shared_from_this<ExynosToneMappingFilter>, public ExynosLog {
public:
    ...
    public:
        ...
        std::shared_ptr<C2StreamHdrDynamicMetadataInfo::input> getHdrDynamicInfo() {
            Helper::Lock lock = mHelper.lock();
            return mHelper.mHdrDynamicInfoInput;
        }
        ...
        static bool IsFilteringEnabled(const std::shared_ptr<C2ComponentInterface> &intf) {
            ...
            if (err != C2_OK && err != C2_BAD_INDEX) {
                StaticExynosLog(Level::Warning, "ExynosToneMappingFilter",
                            "ExynosToneMappingFilter::Interface::IsFilteringEnabled: query failed for %s", intf->getName().c_str());
                return false;
            }
    ...
    private:
        ...
        struct Helper : public C2InterfaceHelper {
            ...
            static C2R HdrDynamicInfoInputSetter(
                    bool mayBlock,
                    C2InterfaceHelper::C2P<C2StreamHdrDynamicMetadataInfo::input> &me) {
                (void)mayBlock, (void)me;

                StaticExynosLog(Level::Debug, LOG_TAG, "[%s] changed", __FUNCTION__);

                return C2R::Ok();
            }

            static C2R HdrDynamicInfoOutputSetter(
                bool mayBlock,
                C2InterfaceHelper::C2P<C2StreamHdrDynamicMetadataInfo::output> &me) {
                (void)mayBlock, (void)me;

                StaticExynosLog(Level::Debug, LOG_TAG, "[%s] applied", __FUNCTION__);
            ...
            std::shared_ptr<C2StreamHdrDynamicMetadataInfo::output>  mHdrDynamicInfoOutput;  /* always be empty */
    ...
    explicit ExynosToneMappingFilter(c2_node_id_t id)
        : mIntf(std::make_shared<Interface>(id)) {
        mbLogOff = false;
        mObjName = "ExynosToneMappingFilter";
        mState = STOPPED;
    }
    ~ExynosToneMappingFilter() override {
...
private:
    void processLoop(std::shared_ptr<ExynosToneMappingFilter> thiz) {
        ...
        while (true) {
            ...
            ExynosLogD("[%s] work #%d : flags=0x%x timestamp=%lld", __FUNCTION__, workCount, work->input.flags, work->input.ordinal.timestamp.peek());
            ...
            if (buffer) {
                ...
                if (buffer->hasInfo(C2StreamHdrDynamicMetadataInfo::output::PARAM_TYPE)) {
                    std::shared_ptr<const C2Info> info =
                        buffer->getInfo(C2StreamHdrDynamicMetadataInfo::output::PARAM_TYPE);
                    std::unique_ptr<C2Param> flipped = C2Param::CopyAsStream(
                            *info, false /* output */, info->stream());
                    hdrDynamicInfo.reset(static_cast<C2StreamHdrDynamicMetadataInfo::input *>(
                            flipped.release()));
                }
                ...
                if (err != OK) {
                    ExynosLogE("[%s] drawLayers returned err:0x%x", __FUNCTION__, err);
                } else {
                    err = fence->wait(500);
                    if (err != OK) {
                        ExynosLogW("[%s] wait for fence returned err:0x%x", __FUNCTION__, err);
                    }
                }
            ...
            ExynosLogD("[%s] sent work #%d", __FUNCTION__, workCount);
        }
    }
...
// static
const std::string ExynosToneMappingFilter::Interface::NAME = "c2.exynos.tone-mapper";
// static
const FilterPlugin_V1::Descriptor ExynosToneMappingFilter::Interface::DESCRIPTOR = {
    // controlParams
...
class ExynosC2FilterPluginComponentStore : public C2ComponentStore, public ExynosLog {
public:
    ExynosC2FilterPluginComponentStore()
        : mReflector(std::make_shared<C2ReflectorHelper>()),
          mIntf(mReflector),
          mFactories(CreateFactories()) {
        mbLogOff = false;
        mObjName = "ExynosC2FilterPluginComponentStore";
    }
    virtual ~ExynosC2FilterPluginComponentStore() = default;

    C2String getName() const override { return "vendor.filter-plugin-store.exynos"; }
...
private:
    ...
    static void AddFactory(std::map<C2String, std::unique_ptr<ComponentFactory>> *factories) {
        ...
        if (!C2InterfaceUtils::FillTraitsFromInterface(traits.get(), intf)) {
            StaticExynosLog(Level::Error, "ExynosC2FilterPluginComponentStore",
                        "[%s] Failed to fill traits from interface", __FUNCTION__);
        }
    ...
    static std::map<C2String, std::unique_ptr<ComponentFactory>> CreateFactories() {
        ...
        AddFactory<ExynosToneMappingFilter>(&factories);
        return factories;
    }
...
class ExynosC2FilterPlugin : public FilterPlugin_V1 {
public:
    ExynosC2FilterPlugin() {
        auto store = std::make_shared<ExynosC2FilterPluginComponentStore>();
        mStore = std::static_pointer_cast<C2ComponentStore>(store);
    }
    ~ExynosC2FilterPlugin() override = default;
    ...
    bool describe(C2String name, Descriptor *desc) override {
        if (name == ExynosToneMappingFilter::Interface::NAME) {
            *desc = ExynosToneMappingFilter::Interface::DESCRIPTOR;
            return true;
        }
        return false;
    }
    ...
    bool isFilteringEnabled(const std::shared_ptr<C2ComponentInterface> &intf) override {
        if (intf->getName() == ExynosToneMappingFilter::Interface::NAME) {
            return ExynosToneMappingFilter::Interface::IsFilteringEnabled(intf);
        }
        return false;
    }
    ...
    c2_status_t queryParamsForPreviousComponent(
        ...
        if (intf->getName() == ExynosToneMappingFilter::Interface::NAME) {
            return ExynosToneMappingFilter::Interface::QueryParamsForPreviousComponent(
                    intf, params);
        }
        return C2_BAD_VALUE;
    }
...
int32_t GetFilterPluginVersion() {
    return ExynosC2FilterPlugin::VERSION;
}

void *CreateFilterPlugin() {
    return new ExynosC2FilterPlugin;
}

void DestroyFilterPlugin(void *plugin) {
    delete (ExynosC2FilterPlugin *)plugin;
}
